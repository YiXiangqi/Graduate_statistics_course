...
::: {.fragment}
对于以上的方程，可以定义一个新的变量，将其转换成我们熟悉的线性模型的形式，比如：
$$
\begin{align}
&y_i = \beta_0 + \beta_1 log(x_i) + \varepsilon_i \\
&z_i = log(x_i) \\
...
于是
$$
y_i = \beta_0 + \beta_1 D_{blue} + \beta_2 D_{green} + \varepsilon_i
$$
...
|Color |D<sub>blue</sub>	|D<sub>green</sub>|
|------|-------|----|
|Red	|0	|0
|Blue	|1	|0
|Green	|0	|1
...
```{r}
#| echo: true
iris_fit <- lm(Sepal.Length ~ Species, data = iris)
summary(iris_fit)
```
...
```{r}
#| echo: false
model.matrix(iris_fit) |> as_tibble() |> rename(
    Intercept = `(Intercept)`
)
```
:::
...
```{r}
#| echo: false
coef(iris_fit) |> as_tibble(rownames = "Term")
```
...
于是
$$
y_i = \beta_0 + \beta_1 D_A + \beta_2 D_B + \varepsilon_i
$$
...
## 多个分类变量：交互效应
如果我们认为 A 和 B 之间存在交互效应，那么模型可以写成：
$$
y_i = \beta_0 + \beta_1 D_A + \beta_2 D_B + \beta_3 (D_A \times D_B) + \varepsilon_i
$$

|A/B |D<sub>A</sub>	|D<sub>B</sub>|D<sub>A</sub> $\times$ D<sub>B</sub>|
|:----:|:-----:|:----:|:-----:|
|a1/b1	|0	|0  |0  |  
|a2/b1	|1	|0  |0  |  
|a1/b2	|0	|1  |0  |  
...
```{r}
penguins_lm_m0 <- lm(body_mass_g ~ species + sex, data = palmerpenguins::penguins)
penguins_lm_m1 <- lm(body_mass_g ~ species * sex, data = palmerpenguins::penguins)
```

```{r}
summary(penguins_lm_m1)
```
...
```{r}
anova(penguins_lm_m1)
```
注意：

- 这里的`anova()`给出的显著性对应于顺序平方和（Sequential Sum of Squares），Type I。
- 对于非平衡数据，不同变量进入模型的顺序会影响`anova()`的结果。
- 对于非平衡数据，建议使用 Type II 或 Type III 的平方和来获取变量的显著性，可以使用`car::Anova()`函数实现。

## `car::Anova()`

- 对于非平衡数据，如果有充足理由认为不存在交互效应，可以使用 Type II 的平方和
- 这时候对于每个变量显著性的解读需要基于**其他变量已经在模型中**的前提下进行。
```{r}
car::Anova(penguins_lm_m1, type = "II")
```
...
## `contrasts()`函数
R 中因子（factor）的默认编码方式是 treatment contrasts，即以第一个水平作为基线水平，其他水平相对于基线水平进行编码。可以使用`contrasts()`函数查看或修改因子的编码方式。例如：
...
指定 sum contrasts 后，重新拟合：
```{r}
#| echo: true
contrasts(penguins$species) <- contr.sum(3)
contrasts(penguins$sex) <- contr.sum(2)
penguins_lm_m1_sum <- lm(body_mass_g ~ species * sex, data = penguins)
car::Anova(penguins_lm_m1_sum, type = "III")
```
...
$$
\begin{align}
&\frac{1}{v_i} = \frac{k_m + s_i}{v_{max}s_i} = \frac{k_m}{v_{max}}\frac{1}{s_i} + \frac{1}{v_{max}} \\ 
\end{align}
$$
...
### *p* 值
这里的 *p* 值对应的零假设（Null hypothesis）是：
...
```{r}
ISIT_gam_m0 <- gam(Sources ~ s(SampleDepth, by = Station), data = ISIT_station_1289, method = "REML")
```
...
#| eval: false
ISIT_station_1289 |>
  mutate(pred = predict(ISIT_gam_m0)) |>
  arrange(Station, SampleDepth) |>
  ggplot() +
  geom_point(aes(SampleDepth, Sources)) +
  geom_line(aes(SampleDepth, pred), linetype = 2) +
  coord_flip() +
  scale_x_reverse() + 
  facet_wrap(~Station)+
  xlab("Depth (m)") +
...
2. `ISIT_gam_m0`与下面模型有何区别。
3. `ISIT_gam_m0`是否需要优化，如何优化？

```{r}
ISIT_gam_m1 <- gam(Sources ~ Station + s(SampleDepth, by = Station), data = ISIT_station_1289, method = "REML")
``` 
