...
pacf 用于测量在控制了所有较短滞后阶数的自相关之后，特定滞后阶数的自相关程度。
...
%%| fig-align: center
flowchart LR
  %% 0: First arrow (Red + pacf on top)
  A["&epsilon;<sub>t</sub>"] -- "pacf" --> C["&epsilon;<sub>t-2</sub>"]
  
  %% 1 & 2: Middle arrows (Blue)
  A --> B["&epsilon;<sub>t-1</sub>"]
  B --> C
  
  %% 3: Last arrow (Blue + acf under)
  %% Note: <br/> is used to push the text below the line
  A --"acf"--> C

  %% Styling the arrows and their text
...
library(gstat)
library(sp)
Boreality_resid_bubble_df <-
  Boreality |>
  mutate(resid = rstandard(Boreality_lm_m0)) |>
  select(resid, x, y)

coordinates(Boreality_resid_bubble_df) <- ~ x + y # coordinates 来自 sp 包

Boreality_resid_bubble_plot <-
  bubble(Boreality_resid_bubble_df, "resid", col = c("black", "grey"))
```
...
## 变异函数（variogram） {.scrollable}

变异函数，(semi-)variogram，是空间统计学中用于描述空间相关性的工具，表示空间数据中变量值之间的差异程度随空间距离的变化情况。
通常有如下定义：
$$
\gamma(\mathbf{h}) = \frac{1}{2} Var[Z(\mathbf{x}) - Z(\mathbf{x} + \mathbf{h})]
$$
其中$h$表示空间距离（lag distance），$\mathbf{x}$表示空间位置向量，$Z(\mathbf{x})$表示位置$\mathbf{x}$处的变量值。
变异函数需要假定 intrinsic stationarity：
$$
E[Z(\mathbf{x}+\mathbf{h}) - Z(\mathbf{x})] = 0  \\
Var[Z(\mathbf{x}+\mathbf{h}) - Z(\mathbf{x})] = 2\gamma(\mathbf{h})
$$
这个假定确保变异函数只是位置之间距离的函数，而与具体位置无关，且不需要假定数据的均值是常数或者有限方差。

## 变异函数与协方差 {.scrollable}
如果进一步假定weak(second-order) stationarity：
$$
E[Z(\mathbf{x})] = \mu \\
Cov(Z(\mathbf{x}), Z(\mathbf{x}+\mathbf{h})) = C(\mathbf{h})
$$
可以将变异函数表示为协方差函数的函数：
$$
\gamma(\mathbf{h}) = C(0) - C(\mathbf{h})
$$
其中$C(0)$表示位置之间距离为0时的协方差，即变量的方差。

## 变异函数的估算 

当我们假定 ergodicity 时, $\gamma(\mathbf{h})$的估计值：

$$
\hat{\gamma}(\mathbf{h}) = \frac{1}{2N(\mathbf{h})} \sum_{i = 1}^{N(\mathbf{h})} (Z(\mathbf{x_i}+\mathbf{h}) - Z(\mathbf{x_i}))^2
$$
其中$N(\mathbf{h})$表示距离为$h$的观测对的数量。


## 变异函数的典型图像 {.scrollable}

::: {.panel-tabset}

### 典型variogram图[^5]

[^5]: 图像来源：Zuur et al., Mixed Effects Models and Extensions in Ecology with R, 2009.

![](images/variogram.png)

### variogram图说明
- 横轴表示位置之间的距离（lag distance），纵轴表示变异函数的估计值$\hat{\gamma}(h)$。
- nugget：？？？
- sill：？？？
- range：？？？
...
在线性回归模型中，误差项满足的假设条件是比 intrinsic stationary 和 weak stationary 更严格的条件，因此残差的变异函数图可以用来诊断误差项的空间相关结构：
...
当误差限之间不存在相关性时，残差的变异函数图应该近似为一个水平线。如果变异函数图显示出随距离增加而增加的趋势，或者存在明显的模式（如周期性），则表明误差项之间存在空间相关性。
...
## 各向同性 {.scrollable}
变异函数通常假定误差项的空间相关结构为各向同性（isotropic）。 
...
### 指数相关结构
指数相关结构（exponential correlation structure）是一种常用的空间相关结构，假设误差项之间的相关性随着距离的增加而以指数方式衰减，可以通过`nlme`包中的`corExp()`函数来指定：
